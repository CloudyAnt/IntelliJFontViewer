...
- **Glyph Index** — Renders every glyph outline via canvas, paginated 64 per page. Filter by Unicode block, Private Use Area (U+E000–U+F8FF), custom range, or exact code point/glyph name; PUA mappings from the font's cmap table are used when available. Click any glyph to open a detail popup with full metrics (name, Unicode, advance width, bearings, bounding box) plus **Copy Text** and **Copy SVG** buttons.
